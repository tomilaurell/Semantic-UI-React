...
If you use scoped CSS in a mixed-style app, custom Semantic UI themes need the same scoping
transform before they are imported. A global custom theme, `semantic-ui-css/semantic.min.css`, or
Semantic UI CDN stylesheet will still style matching elements outside `.semantic-scope`.
...
The same rule applies to scoped CSS: do not load global Semantic UI CSS and scoped Semantic UI CSS
in the same app unless the global leakage is intentional.
